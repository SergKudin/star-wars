...
Детальна документація API знаходиться на сторінці Swagger (Доступна після запуску проекту): [http://localhost:4000/api#/](http://localhost:4000/api#/)
...
В системі передбачена аутентифікація та рольовий доступ.

### Авторизація через JWT токени
Після авторизації, будь ласка, додайте отриманий JWT токен до заголовка авторізації в Swagger, щоб мати доступ до захищених ресурсів.

### Ролі та Права
Система використовує дві ролі: 'admin' та 'user'.
  Роль 'user' має доступ тільки для перегляду загальної інформації.
  Роль 'admin' має повний доступ до всіх можливостей додатку.
### Облікові записи
Кожен користувач може створити свій обліковий запис. Тим не менше, лише 'admin' може змінювати ролі інших користувачів.

### Перший запуск
Під час першого запуску додатку, автоматично створюється обліковий запис з роллю 'admin':

Email: 'master@admin.ua'
Пароль: '123456'
### Зміна Паролю
Після першого запуску рекомендуємо змінити пароль для облікового запису 'master@admin.ua', щоб забезпечити більшу безпеку доступу.
...
4. Перейти за посиланням [http://localhost:4000/api#/](http://localhost:4000/api#/) для доступу до сторінки з документацією, згенерованою Swagger.
...
7. В роботі з фото є деяка особливість. Для створення нового запису спочатку необхідно завантажити зображення. В разі успішного завантаження система поверне створений запис з номером id за яким його вже можна доповнити необною додатковою інформацією.